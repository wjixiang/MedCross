...
def list_databases(
    ...
    return service.list_databases(name_pattern=name, refresh=refresh)
...
def find_database(
    ...
    return service.find_database(name_pattern=name, refresh=refresh)
...
def get_database(
    ...
    """获取 database 详情。"""
    return service.get_database(database_id, refresh=refresh)
...
def list_tables(
    ...
    """列出数据库中的数据表（目录结构）。"""
    return service.list_tables(database_id, refresh=refresh)
...
def list_fields(
    ...
    """列出数据集中的可用字段（精简视图）。"""
    return service.list_fields(database_id, entity=entity, name_pattern=name, refresh=refresh)
